...
// ---- Push token request (with detailed logging) ----
export async function requestPushToken() {
  ...
  try {
    ...
    if (!token) {
      console.warn("[FCM] トークンの取得に失敗しました（getToken が空を返しました）");
      return null;
    }
...
// Calls the Netlify Function that actually dispatches the push.
...
// pushTokens をオブジェクト形式で保存し、配列として返す互換レイヤー
...
// ---- Push notification preferences (object-based token storage) ----
